...
function createRowColumn(type){                                            // Create input for entering no. of rows and columns of a matrix
    ...
    <button id="go" style="padding: 5px;">Go</button>`;
        ...
        if(type==="determinant") order.id = "order_determinant";
        else order.id = "order_inverse";
...
order.addEventListener('submit', (e)=>{
    ...
    if(order.id === "order_add"){
    ...
    else if(order.id == "order_determinant" || order.id == "order_inverse"){
        if(order.id == "order_determinant") n = document.querySelector('#order_determinant #orderOfSquareMatrix').value;
        else n = document.querySelector('#order_inverse #orderOfSquareMatrix').value;
        ...
        if(n === '' || n <= 0 ){
            ...
            if(order.id == "order_determinant") matform.innerHTML+= `<h4>&nbsp;,&nbsp;&nbsp;&nbsp;det(A)</h4><button id="equi" style="margin:5px">=</button>`;
            else matform.innerHTML+= `<h4>&nbsp;&nbsp;&nbsp;&nbsp;A<sup>-1</sup></h4><button id="equi" style="margin:5px">=</button>`;
...
function calculateDet(a,l){
    ...
    for(let i = 0; i<l; i++){
        let submatrix = createSubmatrix(a,0,i,l);
...
function cofactor(a){
    ...
    for(let i=0; i<l; i++){
        cofactorMatrix.push([]);
        for(let j=0; j<l; j++){
            let submatrix = createSubmatrix(a,i,j,l);
            cofactorMatrix[i].push(Math.pow(-1,i+j)*calculateDet(submatrix,l-1));
        }
    }
...
function createSubmatrix(mat, rowToRemove, colToRemove, n){
    let submatrix = [];
    for(let i = 0; i<n; i++){
...
function inverse_matrix(){
    ...
    let d = calculateDet(matrixArr,matrixArr.length);
    if(d == 0) msg.innerText = "Inverse of given square matrix is not possible because det(A) = 0";
    else{
        matrixArr = cofactor(matrixArr);

        transpose(matrixArr);

        divideMatrix(matrixArr,d);

        const c = matrixArr.length;
        Outer: for(let i=0; i<c; i++){
            let r = document.createElement('tr');
            for(let j=0; j<c; j++){
                let data = document.createElement('td');
                data.innerText = matrixArr[i][j];
                r.appendChild(data);
            }
            resultMatrix.appendChild(r);
        }
...
matform.addEventListener('submit', (e)=>{
    ...
    else if(order.id === "order_inverse") inverse_matrix();
